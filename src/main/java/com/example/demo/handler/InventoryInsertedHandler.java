...
@Service
public class InventoryInsertedHandler {
	...
	public void on(InventoryInsertedEvent event) {
		rep.save(new Inventory(Integer.parseInt(event.getInventoryId()), event.getProductId(), event.getSku(),
				event.getQuantity()));
	}

	@EventHandler
	public void on(InventoryUpdatedEvent event) {
		rep.save(new Inventory(event.getInventoryId(), event.getProductId(), event.getQuantity()));
	}
}
